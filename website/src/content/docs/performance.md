...
Measured on a game project (~180 source files, LuaJIT target). Times vary by machine.
...
These benchmarks compare `tstl` (default) vs `optimized` [emit mode](/tslua/config/emit-mode/). Optimized emit mode is early and only covers a few patterns so far (iterator allocation, tostring elision). The numbers below reflect what's implemented today.
